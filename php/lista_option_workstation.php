 ...
 * @return array
 */
function listaWorkstation() : array {

    $dbh = new Db_inc();
    $dbh->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
    $dbh->setAttribute(PDO::ATTR_STRINGIFY_FETCHES, false);
    $query = "SELECT `id` AS `value`, `nome` AS `text` FROM `tb_workstations` WHERE 1 ORDER BY `id` DESC";
...
$lista_workstation = listaWorkstation();

HTTPStatus(200);
HTTPContentType('json');
echo json_encode(array("lista" => $lista_workstation), JSON_PARTIAL_OUTPUT_ON_ERROR);
exit;