...
new Vue({
    ...
    data () {
        return {
            ...
            perPage: 20,
            isBusy: false,
            fields_rendicontazioni: [
                { key: 'data', label: 'DATA', tdClass: 'text-left', sortable: true},
                { key: 'tecnico', label: 'TECNICO', sortable: true, filterByFormatted: true, tdClass: 'text-left'},
                { key: 'codice', label: 'CODICE', sortable: true, tdClass: 'text-right'},
                { key: 'num_ore', label: 'ORE', sortable: true, tdClass: 'text-right'},
    ...
    methods: {
        ...
        showRendicontazioniList(date1, date2){
            this.loading = true;
            this.isBusy = true;
            axios
                .get('./php/lista_rendicontazioni.php?date1=' + date1 + "&date2=" + date2)
                .then(response => {
                    this.rendicontazioniList = response.data.lista_rendicontazioni;
                    this.totalRows = response.data.lista_rendicontazioni.length;
                    this.loading = false;
                    this.isBusy = false;
                });
        },
        ...
        cambiaData(){
            this.showRendicontazioniList(this.date1, this.date2);
        },
        ...
        esportaRendicontazioni(){
            ...
            const options = {
                ...
                data: params,
                url: './php/esporta_rendicontazioni_xlsx.php',
                responseType: 'blob',
            };
            axios(options)
                .then(response => {
                    ...
                    fileLink.setAttribute('download', 'Rendicontazioni_' + this.date1 + '_' + this.date2 + '.xlsx');
